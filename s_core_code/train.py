...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        remaining_time_expected = time_elapsed_ep * (args.hp_ep - epoch - 1)  # -1 for counting

        print("Time elapsed for single epoch: {:.0f}m {:.0f}s".format(time_elapsed_ep // 60, time_elapsed_ep % 60))
        print("Total time expected: {:.0f}m {:.0f}s".format(total_time_expected // 60, total_time_expected % 60))
        print("Remaining time expected: {:.0f}m {:.0f}s".format(remaining_time_expected // 60, remaining_time_expected % 60))
        print("")
...
if args.hp_opt == "sgd":
    optimizer_ft = optim.SGD(model_ft.parameters(), lr=args.hp_lr, momentum=0.9, weight_decay=args.hp_wd)

if args.hp_opt == "adam":
    optimizer_ft = optim.Adam(model_ft.parameters(), lr=args.hp_lr, weight_decay=args.hp_wd)
...
if args.mode in ["train"]:
    # return이 best model임
    model_ft, best_acc = train_model(model_ft, criterion, optimizer_ft, scheduler, num_epochs=args.hp_ep)

    save_dir = "retrain_model_folder/pt_" + str(args.model_name) + "_lr" + str(args.hp_lr) + "_wd" + str(args.hp_wd)
